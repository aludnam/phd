...
\begin{figure}[!htb]
	...
	\caption{Simulated PSF in different depths of focus.  The number in each figure indicates the distance in $\um$ from the in-focus plane. (a) Intensity scaled to the in-focus PSF. (b) The intensity was scaled in each freame. The maximum intensity relative to the in-focus PSF is indicated in the bars below and corresponds to about 10\% at $1\um$ and 3\% at $1.5\um$.}
...
\Autoref{sub:results - comparison} shows a qualitative comparison of \inmf\ results with competititve techniques CSSTORM and 3B analysis dealing with overlapping sources. Average precison was used as a quantitative measure of the algorithms performance on simulated datasets consisting of randomly scattered sources with different densities.
...
To achieve a smoother representation of the unerlying structure, we used the sum of ten \inmf\ evaluations (see discussion in \autoref{sub:visualisation}) rather then only one \inmf\ run used for the data of randomly scattered sources in the secton above. The result is visualised in \autoref{fig:comparison hash dens 12}\bbb. The same dataset was evaluated with 3B and CSSTORM, shown in  \autoref{fig:comparison hash dens 12}\ccc\ and \autoref{fig:comparison hash dens 12}\ddd, respectively. 
