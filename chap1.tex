...
%	\includegraphics[width=\widthfig]{\qd S569/figures/dpixc_1to8_dens3}}
	...
	\subfloat[linear density $\mu=6\unit{\mu m^{-1}}$, distance between lines $d=150\unit{nm}$]{
	\includegraphics[width=\widthfig]{\qd S569/figures/dpixc_1to8_dens6}}
	
	\subfloat[linear density $12\unit{\mu m^{-1}}$, distance between lines $d=150\unit{nm}$]{
	\includegraphics[width=\widthfig]{\qd S569/figures/dpixc_1to8_dens12}}		
	
	\subfloat[linear density $10\unit{\mu m^{-1}}$, distance between lines $d=100\unit{nm}$]{
...
\begin{figure}[!hbt]
	...
	\caption{Illustration of the $\bm{w_{k}}$s ``squeezing''. Eight (out of 60) selected $\bm{w_{k}}$s  shown. The number in the top left corner is the index in the $L_{2}$ norm sorted $\bm{w}$s.}
...
The average precision \autoref{eq:AP} for four blinking behaviours from \autoref{fig:blinking} is shown in \autoref{fig:AP blinking}. The mean and the standard deviation of the results from five different geometric configuration of randomly scattered sources is shown. 

We observe that the AP is proporitonal to the variance of the blinking. NMF is not taking the time-series structure into account. Data sets with permuted time frames are equivalent and NMF will give identical results. This is certainly drawback of the NMF model, because the correlations between the adjecent time frames can provide usefull infromation. Note that 3B algorithm  \cite{Cox2011} exploits this information by modelling the blinking behavior of the fluorophores by a Markov chain.
...
\begin{figure}[!htb]	
	...
	\caption{(a) Estimated density and (b) average precision as a function of number of subsampled frames. Mean and standard devaition of three different geometrical configurations of the randomly scattered sources is shown.}
	\label{fig:AP subsampled}
\end{figure}

\fixme{make more evaluations}
...
\begin{figure}[!p]
	...
	\caption{Comparioins of the results for simulated of randomly scattered sources with density $10\unit{\mu m^{-2}}$ ($14$ sources in total). Sum projection of the dataset with true sources' positions marked with red dots is shown in (a). Red circles show the true locations of the sources. The radius of the circles $r=0.7\unit{pixels}$ ($56\unit{nm}$) indicates the true-positive threshold distance. For further information see \autoref{sec:evaluation}. Scale bar 400\unit{nm}.}
...
\begin{figure}[p]
	...
	\caption{Comparison of the results for simulated of randomly scattered sources with density $40\unit{\mu m^{-2}}$ ($14$ sources in total). Sum projection of the dataset with true sources' positions marked with red dots is shown in (a). Red circles show the true locations of the sources. The radius of the circles $r=0.7\unit{pixels}$ ($56\unit{nm}$) indicates the true-positive threshold distance. For further information see \autoref{sec:evaluation}. Scale bar 400\unit{nm}.}
...
3B completely fails to recover the double parallel lines, replacing them with one intensity crest in between of the lines (blue arrow in \autoref{fig:comparison hash dens 12}d). CSSTORM shows the double line structure of the hash symbol at the perifery of the specimen (green arrows in \autoref{fig:comparison hash dens 12}c), however the double lines joins into a single line close to the center of the cross  (blue arrow in \autoref{fig:comparison hash dens 12}c). The hole ($150 \times 150 \unit{nm}$) in the middle of the specimen is completely unresolved and is replaced by an intensity maximum (red arrow in \autoref{fig:comparison hash dens 12}c). 

NMF shows the double line sturcure all the way along the arifical specimen (green arrows in \autoref{fig:comparison hash dens 12}b) and  the hole in the middle of the structure is clearly visible (red arrow in \autoref{fig:comparison hash dens 12}b). \Autoref{fig:demo pow w result}d demonstrates that the double lines and the hole in the middle can be observed even for a structure with lines as close as $d=100\unit{nm}$ ($1.25 \unit{pixels}$). 
...
\begin{figure}[!h]
	...
	\caption{Evaluation of the aritfical structure data with three different methods. The paralel lines are separated by $d=150 \unit{nm}$ ($1.8 \unit{pixels}$). The sources are distributed along the lines with a linear density $12 \unit{\mu m^{-1}}$. Arrows in a wide-field image (a) points at sub-resolution features of the specimen. Scale bar 400\unit{nm}.}
...
We applied the pipeline described in \autoref{sec:NMF-for-real} to a stack of $T=1000$, $128\times128$ images of tubulin fibres labelled with QDs. The parameters of the experiment are shown in \autoref{tab:parameters experiment}.
...
\end{figure}
%
The dataset was divided into $25 \times 25$ patches (\autoref{fig:Patches}), and only patches with sufficiently strong signal (thick boxes in \autoref{fig:Patches}) were considered for further evaluation. The number of sources within each patch is over-estimated via principal components analysis (\autoref{sub:Estimation-of-number-of-sources}). Each patch was evaluated using the NMF with iterative restarts (\autoref{alg:restarts}).
...
\begin{figure}[!htb]
	...
	\caption{Real data - patch \texttt{B24} from \autoref{fig:Patches}. (a) 14 randomly selected frames (out of $10^{3}$) of the tubulin structure stained with QDs. (b) Estimated sources $\bm{w}_{k}$ sorted according to their $l^{2}$ norm (shown all $K=56$ sources). Bars below each figure show the maximum of the $\bm{w}_{k}$ intensity image.}
...
\begin{figure}[!htb]
	...
	\caption{Wide field (WF) and NMF results for regions marked in \autoref{fig:tubulin WF and NMF}. NMF results are shown in false colors to enhance the contrast of dim features. Scale bars $200 \unit{nm}$.} 
	\label{fig:tubulin details}
\end{figure}
% 
Each patch was evaluated five times with different initial radom restarts. The results were visualised by ``squeezing'' $\bm{w}$s described in \autoref{sub:visualisation}, using the over-sampling by a factor $r=4$ and the power $p=30$. The images for five different evaluatins were summed together to create a sub-resolution image. The final image of the whole dataset was created by tiling results for the individual patches. The border pixels of each patch was removed to avoid overalps of the results.  

\Autoref{fig:tubulin WF and NMF} compares the wide-field (WF) image with NMF evaluated results. The close-up of the highlighted regions in \autoref{fig:tubulin WF and NMF}a for WF and NMF is displayed in \autoref{fig:tubulin details} (using false colors to enhance contrast of the dim features in the NMF results). The sub-resolution structures were recovered with NMF. 
