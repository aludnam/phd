...
However, $\bm{o}$ and $\bm{w}$ represent different objects then the matrices $\bm{W}$ and $\bm{H}$ in the NMF model. In the RL deconvolution one PSF $\bm{w}$ is shared by all sources. In the NMF each source has its own PSF (columns of $\bm{W}$). 
