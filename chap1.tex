...
\begin{figure}[!htb]
	...
	\end{tabular}	
	\caption{Selection of the credible $\bm{w}$s. The green and blue boxes indicates the estimated ``credible'' sources (with only one major global maximum). The sources with blue frame have the maximum closer then 2 pixels to the border and can be therefore considered as partly missing sources. The sources with red frame have two local maxima of comparable strength. Bars under the figures show the maximum of the estimated $\bm{w}_{k}$.}
	\label{fig:good w}	
\end{figure}
...
\begin{table}[!h]
	...
	\begin{tabular}{|c|c|l|}
		...
		\hline 
		$b$ 				& background/pixel 				& 100\tabularnewline
		\hline
	\end{tabular}
...
The subsampling of the data gives higher singnal-to-noise rate in each frame, however the sources appear to be more in the ``ON'' state because of their ``OFF'' periods get averaged out. 
