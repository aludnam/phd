...
Because each frame of the dataset is treated independently, these methods ignore the fact that some sources can stay ON for several succesive frames or can even reappear in different frames due to blinking. These can generally deal with moderately overlapping sources with densities $<10\unit{sources/\mu m^{2}}$ \cite{Huang2011,Holden2011,Zhu2012}.
...
We also develop methods to reduce local optima problems in the fitting procedure (\inmf\ alogrithm discussed \autoref{sub: Iterative restarts}). 

The results of the NMF can be used in two diffrerent ways. The separated individual sources $\bm{w}$ can be localised and the estimated positions can be used either directly or to create a sub-resolution image very much like in the conventional LM techniques (see \autoref{sub:Localisation-and-stitching}). A different approcach avoids the localisation step and creates the super-resolutin image directly from the estimated $\bm{w}$s by combining their ``squeezed'' version (see \autoref{sub:visualisation}). 
...
\begin{enumerate}
	...
	\item
	Number of local maxima bigger than $50\%$ of the global maximum is counted. The threshold $50\%$ is arbitrary and it reflects our empirical experience that the peaks with intensity less then half of the brightest peak's intensity are not very visible in the scaled image of $\bm{w}$.
\end{enumerate}
...
\begin{figure}[!htb]
	...
	\end{tabular}	
	\caption{Selection of the credible $\bm{w}$s (here ordered by $L_1$ norm). The green and blue boxes indicate the estimated ``credible'' sources (with only one major global maximum). The sources with blue frame have the maximum closer than two pixels to the border and can be therefore considered as partly missing sources. The sources with red frame have two local maxima of comparable strength. Bars under the figures show the normalised maximum value of the estimated $\bm{w}_{k}$ multiplied with mean brightness of the source estimated from the intensity matrix $\bm{H}$. The true number of sources in this simulated dataset was $K_{true}=72$.}. 
	\label{fig:good w}	
\end{figure}
%
...
As we noted earlier in this section, the calssification method dicussed above is limited to the to the in-focus PSFs (with only one major local maximum of the intensity). To accomodate for the individually different shape of the PSF, such as out-of-focus PSF (right side of \autoref{fig:Simulted-PSF-different-focal-depths}) we have to use a different approach. 

One possibility is to compute a set of ``features'' on each estimated $\bm{w_k}$ (and possibly on corresponding $\bm{h_k}$) and use a linear classifier to identify the class of each estimated source. The possible ``features'' can include, for example, $L_2$ norm, number of clusters in the thresholded image, maximum of the cross-correlation with the PSF, measure of smoothness of the estimated result, distance of the global maximum from the edge and many others. Each $\bm{w_k}$ then represents a point in a high-demensional feature space. The linear classifier assumes that the individual classes can be separated with linear manifolds in the feature space.

However, the linear classifier has to be trained on a set of labelled data. The training therefore requires a manual labelling of at least several hundreds of $\bm{w}$s (assignment of the class to each $w_k$). In an ideal world, one training set would be sufficient for different datasets. The calsifier, once trained, would be applicable for results from diffrent datasets taken in a range of experimental conditions. However, our experience is that the classification performance varies significantly with change of the experimental parameters (size of the patch, pizel-size, background levels). 
...
Oversampling of $\bm{w_{k}}$s (for example zero-padding of the Fourier transform of the $\bm{w_k}$'s image) is needed before taking the higher powers $p$. \Autoref{fig:demo pow w} shows the original estimated $\bm{w_{k}}$s and the corresponding up-sampled (by a factor of $r=4$) $\bm{w_{k}}$s taken to the power $p=5$. 
...
If we normalise the $L_{1}$ norm of $\bm{w}^{p}$ to 1 ($\sum_{x} \bm{w}^{p}(x)=1$), we can reconstruct a ``super-resolution'' image by summing all $\bm{w_{k}}^{p}$, weighted by the corresponding mean intensity $\unit{mean}(\bm{h_{k}})$. The results for different values of $p$ are shown in \autoref{fig:demo pow w result} and can be compared with the conventional visualisation in \autoref{fig:visualisation gaussf}\ccc,\ddd.
...
\Autoref{sub:results - comparison} compares \inmf\ with CSSTORM and 3B analysis, techniques which can deal with overlapping sources.
...
\begin{description}
	...
	\item[3B analysis:]
	More than $10$ hours for $\sim30$ iterations.
\end{description}
...
\clearpage
\subsection{Comparison with other methods - aritfical structure\label{sub:results - comparison - structure}}
...
\begin{figure}[!h]
	...
	\caption{Evaluation of the artificial structure data with three different methods. The parallel lines are separated by $d=150 \unit{nm}$ ($1.8 \unit{pixels}$). The sources (indicated as red dots in (a)) are distributed along the lines with a linear density $15 \um^{-1}$. Arrows in a wide-field image (a) point at sub-resolution features of the specimen (futher discussed in the text). Scale bar 400\unit{nm}.}
	\label{fig:comparison hash dens 12}
	% there is a Richardson lucy deconvolution of this dataset in S570/figures ....
\end{figure}
%
The mean of ten \inmf\ evaluations with iterative restarts is visualised in \autoref{fig:comparison hash dens 12}\bbb. The same dataset was evaluated with 3B and CSSTORM, shown in  \autoref{fig:comparison hash dens 12}\ccc\ and \autoref{fig:comparison hash dens 12}\ddd, respectively. 
...
\end{figure}
%
The dataset was also evaluated with Richardson-Lucy (RL) deconvolution ({\tt deconvlucy} function in MATLAB) with provided known (true) PSF and run for 500 iterations. RL of the sum projection of the dataset is shown in \autoref{fig:Cross RL SOFI}\aaa. We also used RL deconvolution to each frame of the dataset. The sum projection of deconvolved frames is shown in \autoref{fig:Cross RL SOFI}\bbb. 

\Autoref{fig:Cross RL SOFI}\ccc\ shows the application of the second order SOFI image (\cite{Dertinger2010b} and discussion in \autoref{sec:Overlapping sources}). The second order SOFI corresponds to the variance of the pixels intensity along the frames. 

Neither of these methods was capable of discriminating the sub-resolution features of the artificial struture (compare with \autoref{fig:comparison hash dens 12}).
