...
\end{equation}
%
where $C_{2}$ is independent of $\bm{W}$and $\bm{H}$. Comparison with the log-likelihood eq.~\autoref{eq:NMF model likelihood} shows that the minimum of the divergence with positivity constrains on $\bm{W}$ and $\bm{H}$ is equivalent to the maximum of the log-likelihood. 
...
NMF with explicit sparsity constraints has been developed by Hoyer  \cite{Hoyer2004}. Specific constraints on ``sparsity'', defined in  \cite{Hoyer2004}, of the columns of $\bm{W}$ can be imposed during the optimization. 

All the columns $\bm{w}_{k}$s of estimated matrix $\bm{W}$ have an identical ``sparsness'' which might be restrictive when out-of-focsu PSFs are present. For example, the in focus PSF in \autoref{fig:Simulted-PSF-different-focal-depths} has Hoyer's sparsity $s=0.83$ while the PSF from $1 \unit{\mu m}$ out-of-focus plane has $s=0.4$ and the PSF from $1.8 \unit{\mu m}$ out-of-focus plane  has $s=0.1$.
%
...
\begin{figure}[!hbt]
	...
	\caption{Histograms of the $K$ estimations with analysis of correlations in residuals (a) and varaiational lower bound (b). Simulated data with three different sources density: 10 sources within a disk of $\delta$ (blue), $1.5\delta$ (green) and $2\delta$ (red).}
...
\emph{Say more here when we are sure of what is  done, e.g.\ classifier, PALM source identification etc ..} \fix

When dealing with mostly in-focus PSFs it is possible to use 
...
\end{align*}
%
An example of precision $P(l_{i})$ and recall $R(l_{i})$ curves for different confidence levels is shown in \autoref{PRcurve}a. 
%
\begin{figure}[!h]
	...
	\caption{(a) Example of the precison $P(l_{i})$ (blue) and recall $R(l_{i})$ (green) curve. (b) The precison/recall curve $P(R)$ (blue) with interpolated precision $P_{interp}(\tilde{R}$) (red).}
	\label{PRcurve}
\end{figure}


The precision/recall (PR) curve $P(R)$ (\autoref{PRcurve}a) is interpolated for $11$ equally spaced recall levels $\tilde{R}_{i}\in[0:.1:1]$ by taking the maximum precision for which the corresponding recall exceeds $\tilde{R}_{i}$ (\autoref{PRcurve}b):
%
\begin{equation}
	P_{interp}(\tilde{R})=\max_{R;R\geq \tilde{R}}P(R)
\end{equation}
%
The precision/recall (PR) curve is interpolated in order to reduce the impact of ``wiggles'' in the PR curve (see \autoref{PRcurve}b). Note that to obtain a high AP, the method must have precision at all levels of recall penalysing methods that can accurately estimate only few very bright sources. 
% ``wiggles'' in the precision/recall curve, caused by small variations in the ranking of examples. 

Average precision (AP) is a mean of interpolated precsion.
%
\begin{equation}
...
To test the performance of the algorithm in different experimental settings we created several simulated datasets. The parameters of the simulations such as size of the PSF and the brightness of the fluorophores are chosen to correspond to a real QD data and are sumarised in \autoref{tab:Simulations parameters}. 
%
\begin{table}[!h]
	\centering
	\begin{tabular}{|c|c|l|}
		...
		\hline 
		$\left< n_{phot} \right>$ & mean number of photons / source / frame 	& 2500\tabularnewline
		\hline 
...
\begin{enumerate}
	...
	\item
	Telegraph proces with switching rate $\gamma$: the blinking was oversampled $100\times$, telegraph process was generated between $0$ and $\max(n_{phot})$ with $\gamma/100$ and then undersampled $100\times$. This way some intermediate values of the intensity are genereated in the blinking. 
\end{enumerate}

The blinking behaviour is discussed more in details in \autoref{sub:blinking behavior}.


 
%==========================================

\subsection{Randomly scattered sources}
...
Results of the simulted data are presented first. Average precisoin described in \autoref{sub:AP} is used as a performance measure. 

%==========================================

\subsection{Effect of the source density and blinking behaviour \label{sub:blinking behavior}}
...
For low densities ($10\unit{\mu m^{-2}}$) the results reach the plateay AP about $90\%$ for 100 frames (blue curve in \autoref{fig:AP on frames}). Adding more frames does not increase AP. Higher densities of 

%==========================================

\subsection{Comparison with other methods}
\fix
\emph{compare some simulations with 3B and CSSTORM}

%==========================================

\subsection{Different individual PSFs}
%
\begin{figure}[!htb]	
...
\subsection{Real data - out of foucs PSFs}
