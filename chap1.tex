...
\begin{enumerate}
	...
	\item
	Discussion of Hoyer's sparse NMF.  Show on simulated in focus PSF - all have the same sparsity. (Simulated data with in-focus PSF randomly scattered \texttt{S433}, with different PSFs \texttt{S401}. Real data with Hoyer in \texttt{S402}). Berry an Appendix A from the first year review. 
	\item
	...
	\begin{itemize}
		\item
		Randomly scattered PSFs with different densities (range $0-50 \unit{um^{-2}}$). Evaluate AP, estimated density, localisation error. Show limits of the method. (Density, intenisty, blinking behaviour, number of frames.)
		\item
...
\subsection{Hoyer's sparse NMF}

NMF with explicit sparsity constraints has been developed by Hoyer  \cite{Hoyer2004}. Specific constraints on sparsity (defined in  \cite{Hoyer2004}) of the columns of $\bm{W}$ can be imposed during the optimization. All the comlumns $\bm{w}_{k}$s of estimated matrix $\bm{W}$ have an identical `sparsness' which might be restrictive when out-of-focsu PSFs are present.  

For example, the in focus PSF in \autoref{fig:Simulted-PSF-different-focal-depths} has Hoyer's sparsity $s=0.83$ while the PSF from $1 \unit{\mu m}$ out-of-focus plane has $s=0.4$ and the PSF from $1.8 \unit{\mu m}$ out-of-focus plane  has $s=0.1$.
...
The NMF model \autoref{eq:NMF model - elemen-wise} requires prior knowledge about the number of sources $K$ to be separated. This is a difficult task for noisy datasets. In preliminary work we explored this on simulated data with models fitted for a range of $K$ values. \autoref{fig:K estimation}(a-c) represents three different simulated datasets with sources randomly scattered within an area of radius $2\delta$, $1.5\delta$ and $\delta$, respectively. $\delta$ was equal to diameter of an Airy disk, shown as a green circle in \autoref{fig:K estimation}(a-c) ($\delta=0.61\frac{\lambda_{em}}{NA}$). Each image shows the mean of all frames of the dataset and correspond to a wide-field image with blue circles indicating the true positions of the sources.
...
\begin{enumerate}
	...
	A crude estimation of $K$ can be obtained from a position of a ``kink'' in the plot of sorted principal values \autoref{fig:K estimation}d-f. However, the ``kink'' is not obvious in the noisy data or data with high density of blinkink sources \autoref{fig:K estimation}f.
	\item
	\emph{A variational lower bound which approximately integrates out $\bm{H}$ \cite{Buntine2006}.} 
	
	A variational approximation of the GaP model \autoref{sub: GaP} provide lower bound $\mathcal{L}$ on the likelihood $p(\bm{D}|K, \theta)$ by approximately integrating out the latent variables $\bm{h}_{k}$. To obtain the marginal likelihood $p(\bm{D}|K)$ it would be necessary to also integrate out $\theta$, but this is computationally challenging. We show that in fact the lower bound already underestimates the value of $K$, so that $p(\bm{D}|K)$ would likely peak at even lower values of $K$ and thus systematically underestimate the number of sources \autoref{fig:K estimation}g-i.
...
The iterative procedure is illustrated on simulated data in \autoref{fig:Iterative restarts}. For further explanation of the figure see \autoref{sec:results}.

%==========================================

\subsection{Classification of the estimated sources\label{sub:Classification-of-sources}}

The estimated sources can greatly vary in quality. While some $\bm{w}$'s are a credible representation of a PSF, there are often $\bm{w}$'s which contain multiple PSFs. Because of the overestimation of the number of sources $K$ (\autoref{sub:Estimation-of-number-of-sources}), there are also $\bm{w}$'s present which correspond to background noise. Sources located close to the patch border and therefore partially missing should also be identified; these sources will likely appear in the adjacent patch entirely, because the overlap of the patches is set to approximately the extent of the (in focus) PSF.

\emph{mention the non-maximum suppression} \fix

\emph{Say more here when we are sure of what is  done, e.g.\ classifier, PALM source identification etc ..} 
\fix

When dealing wtih mostly in-focus PSFs it is possible to use 
...
The performance of the algorithm applied on a simulated dataset can be quantitatively measured, because the true locations of the sources is known. We used several measures to compare the algorithm performance on simulated datasets consisting of randomly scattered in-focus PSFs. 
...
The individual estimated sources $\bm{w_{k}}$ were localised by ML fitting of a gaussian approximation of a PSF \cite{Zhang2007}. We used a greedy algorithm to assign the esimated locations ($E$) to the nearest true position ($T$). Only one estimated position was assighend to one true position. If the distance between and estimated and the true position was smaller than a threhold $r$, then the source was considere as a true positive (TP). A true position with no esitmated source within a disk with radius $r$ was false negative (FN), whereas an estimated position further then $r$ from any true position was considered as false positive (FP) \autoref{TPFPFN}. $M$ estimated sources in the proximity of one true source are counted as $1$TP and $(M-1)$FP (\autoref{TPFPFNcombi}, left). One esimated source in proximity of $M$ true sources gives $1$TP and $(M-1)$FN (\autoref{TPFPFNcombi}, right).

\begin{figure}[!h]
	\centering
	\includegraphics[width=1.0\textwidth]{figures/TPFNFPillustration/TPFNFPs_comb}
	\caption{There is only one estimated position assigned to each true position. Two estimated sources in the proximity of one true source are counted as $1$TP and $1$FP (left). One esimated source in proximity of two true sources gives $1$TP and $1$ FN (right)}
	\label{TPFPFNcombi}
\end{figure}
...
Localisatoin error was estimated as an average distnace between true locations  and estimated locatins classified as TP. 

\subsection{Estimated density}
The estimated density of the sources was counted as number of TP divided by the area of the image.

\subsection{Average precision}
The average precision (AP) \cite{Salton1986,Everingham2009} was used to sumerise both localisation precision and ability to recover the individual sources.

The estimated positions $e_{k}$ are ranked according to the square of the source's mean brightness $b_{k}$. This is retrieved from the matrix $\bm{H}$ as a mean along the rows. (The localisation precisoin should scale as $1/sqrt(N)$, where $N$ is a number of photons emmited by a source.
...
Precisioin $P$ and recall $R$ are computed from $\unit{TP}(l_{i})$, $\unit{FP}(l_{i})$ and $\unit{FN}(l_{i})$ for each confidence level $l_i$.
...
"The precision at each recall level $\tilde{R}$ is interpolated by taking the maximum precision measured for a method for which the corresponding recall exceeds $\tilde{R}$."
\begin{equation}
...
\begin{figure}[!h]
	...
	\caption{(a) Precison - recall curve (blue). Interpolated precision in red. (b) Interpolated precision.}
	\label{PRcurve}
\end{figure}


\section{Average precison AP}
\begin{equation}
AP=\frac{1}{11}\sum_{\tilde{R}}{P_{interp}(\tilde{R})}
\end{equation}
In the case shown in fig.~\ref{PRcurve} is $AP=67\%$


%==========================================
\section{Results \label{sec:results}}

\subsection{Iterative restarts}
%
\begin{figure}[!htb]	
	...
	\begin{tabular}{l}
		\noindent		
		\includegraphics[width=\widthfig]{\qd S382/images/wtrue_l2sort}\tabularnewline
	...
	\begin{tabular}{l}
		\includegraphics[width=\widthfig]{\qd S382/images/wtrue_l2sort_noise}\tabularnewline
		\includegraphics[width=\widthfig]{\qd S382/images/wtrue_l2sort_noise_intBars}\tabularnewline
	\end{tabular}}\\	
	\subfloat[Estimated sources]{
	\begin{tabular}{l}
		\includegraphics[width=\widthfig]{\qd S382/images/resw_1to8}\tabularnewline
...
\subsection{Effect of the number of frames}

\autoref{fig:AP on frames}
...
\subsection{Effect of the source density and blinking behaviour}
...
\begin{figure}[!h]
	...
	\caption{Examples of blinking behavior (50 out of 1000).}
\end{figure}
	
\begin{figure}[!h]
	\centering
	\includegraphics[scale=.6]{\qd S455/images/blinkmat_varS422S445S450S455}
	\caption{Varinace of hte blinkmat for different simulations.}
\end{figure}
