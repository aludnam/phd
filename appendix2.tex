...
\end{equation}
%
and therefore Fisher Information for Poisson distributed data becomes
%
\begin{equation}
...
\end{equation}
%
where $q$ is the PSF of the sources. For pixelated version (integral over pixel area $\Gamma_k$) with homogeneous background $b$ in each pixel the intensity can be expressed as:
%
\begin{equation}
...
\end{alignat}
%
where we have set $q_k(z)=\int_{\Gamma_k}q(x-z)dx$ as the pixelated version of a point spread function translated by $z$ with $\Gamma_k$ being an area of the $k$th pixel, and $q'_k(z)=\int_{\Gamma_k}\frac{\partial q(x-z)}{\partial x}dx$ as the corresponding pixelated derivative.
