...
This is derivation of the Fisher information for Poisson distributed variable $X$ with mean $\lambda(\theta)$:
...
The Fisher information can be expression in these equivalent forms:
\begin{equation}
...
\end{alignat}
%
Because $n_k$ are independent identically distributed (i.i.d.), the second term can be expressed as 
%
\begin{equation}
	\E\left[\sum_k\sum_{m\neq k}\frac{\partial \LL_k}{\partial\theta}\frac{\partial \LL_m}{\partial\theta}\right] 
...
\end{equation}
%
we get for the Fisher information
%
\begin{alignat}{2}
	I(\theta)
...
\end{alignat}
%
We recognise the variance $\var(n_k)=\E\left[(n_k-\lambda_k)^2\right]$. For Poisson variable 
%
\begin{equation}
	\var(n_k)=\mathrm{mean}(n_k)=\lambda_k,
\end{equation}
%
and therefore the Fisher Information for Poisson distributed data becomes
%
\begin{equation}
...
This section comments on the Fisher Information estimation as described by Ram et al. in \cite{Ram2006}.

For two sources separated by a distance $d$ we have the mean value of the intensity:
%
\begin{equation}
	\lambda(x)=\Lambda_1f_1(x)+\Lambda_2f_2(x),
\end{equation}
%
where $f_i$ and $\Lambda_i$ is the response function and the intensity, respectively, of the source $i$. For translationally invariant PSF and in-focus sources: $f_1=q(x-\frac{d}{2})$ and $f_2=q(x+\frac{d}{2})$
%
\begin{equation}
...
\end{equation}
%
where $q$ is the PSF of the system. For pixelated version (integral over pixel area $\Gamma_k$) with homogeneous background $b$ in each pixel the intensity can be expressed as:
%
\begin{equation}
	\lambda_k(d)=\Lambda_1\int_{\Gamma_k}q(x-\frac{d}{2})dx+\Lambda_2\int_{\Gamma_k}q(x+\frac{d}{2})dx+b.
\end{equation}
%
By plugging into \autoref{eq:app-Fisher Info for Poisson} we obtain expression for the Fisher Information:
%
\begin{alignat}{2}
	I(d)
...
\end{alignat}
%
where we have set $q_k(z)=\int_{\Gamma_k}q(x-z)dx$ as the pixelated version of the point spread function translated by $z$. $\Gamma_k$ is the area of the $k$th pixel, and $q'_k(z)=\int_{\Gamma_k}\frac{\partial q(x-z)}{\partial x}dx$ is the corresponding pixelated derivative.

FREM is defined as a lower bound on the standard deviation ($\sqrt{\var(d)}$) of the source separation $d$ estimator
%
\begin{equation}
...
When the sources are far apart, the mixing term in the nominator in \autoref{eq:app-Fisher Info Pixelised - Ram} vanishes: $\Lambda_1\Lambda_2\partial_{x}q(x-\frac{d}{2})\partial q(d+\frac{d}{2})=0$ because $\partial_{x}q(x-\frac{d}{2})$ and $\partial_{x}q(x+\frac{d}{2})$ do not have common overlap. \Autoref{eq:app-Fisher Info Pixelised - Ram} then decomposes into two individual terms
%
\begin{equation}
	I(d) 
...
\end{equation}
%
This corresponds to the sum of the Fisher Information for localisation of individual sources. 

\subparagraph*{Situation with missing source $\Lambda_i=0,\ \Lambda_j\neq0$:}\ \\
Even if one of the source is missing the Fisher information is strangely non-zero $I(d)\neq0$. The variance remains finite even if one of the sources is not present! This is a consequence of the assumption that the sources are located symmetrically at $\pm d/2$ with respect to the origin. Only one source is then needed to determine the distance $d/2$.

%==========================================
%==========================================

\section{An alternative way to derive FREM}
\label{sec:Appendix FI alternative}
Below we show how to fix the problems with the original FREM. Our FREM gives infinite variance when one of the sources is not present and fixes the strange behaviour of the unequal sources for the limit $d\rightarrow0$. 
...
\end{equation}
%
where $\bm{Q}$ is the covariance matrix. The lower bound on $\bm{Q}$ is expressed as the inverse of the Fisher information matrix 
%
\begin{equation}
...
\end{equation}
%
is given by generalisation of \autoref{eq:app-Fisher Info for Poisson}
%
\begin{equation}
...
\end{equation}
%
Note that the Fisher information matrix is symmetrical $I_{12}=I_{21}$ due to exchangeability of the derivatives.
...
\end{equation}
%
where $q_k(c_i)$ is the pixelated version (pixel area $\Gamma_k$) of the PSF
%
\begin{alignat}{2}
...
We now consider the limits of very close sources ($d\rightarrow 0$) and well-separated emitters ($d\rightarrow\infty$) for FREM.
...
\end{equation}
%
which can be further simplified by explicitly substituting $f_k(c_1,c_2)$
%
\begin{equation}
...
\end{equation}
%
for any $\Lambda_i$. $\bm{I}(\theta)$ is therefore a singular matrix for $d=0$ and inverse $\bm{I}^{-1}(\theta)$ does not exist for $c_1=c_2$. However,  the limit $c_1\rightarrow c_2,\,(d\rightarrow0)$ gives $p\neq0,\, r\rightarrow0$ and FREM$=\frac{p}{r}\rightarrow\infty$. FREM therefore diverges for sources with any combination of source intensities $\Lambda_1$ and $\Lambda_2$. This is in contrast to the original FREM formula \autoref{eq:app-Fisher Info Pixelised - Ram} which gives diverging FREM only for equally strong sources $\Lambda_1=\Lambda_2$. 

\subparagraph*{Well-separated sources limit $d\rightarrow\infty$:}\ \\
The cross term $I_{ij}$ in \autoref{eq:app-Fisher Information alternative - Individual} vanishes ($I_{ij}=0,\: i\neq j$) because of the multiplication $q'_k(c_1)q'_k(c_2)$, which is very close to zero for well-separated PSFs. This assumes that the PSF (and its first derivative) decreases to negligible values for distance far from the centre of the PSF. Then from \autoref{eq:app-variance alternative} 
%
\begin{equation}
	\var(d)\geq\frac{1}{I_{11}}+\frac{1}{I_{22}},
\end{equation}
%
which is the sum of bounds on variances for localisation of two individual sources:
%
\begin{alignat}{2}
	I_{ii}
...
\end{alignat}
%
We used the fact that the PSFs $q(c_1)$ and $q(c_2)$ are well-separated and decrease (with its first derivatives) towards zero in the regions far from their centre. Therefore the $q(c_j)$ is negligible in the region where $q'(c_i)$ ($j\neq i$) have any significant values. The term $\Lambda_jq(c_j)$ in the denominator can be therefore neglected. 
...
\end{equation}
%
These terms correspond to the localisation of the individual sources. The ``localisation precision'' for one sources $s_i$ is then bounded by $\sigma/\sqrt{\Lambda_i}$. This corresponds to the ``squeezing'' of the initial `` localisation uncertainty'' $\sigma$ by the square root of the source's intensity. 

For well-separated sources is then the lower bound on the variance give by
%
\begin{equation}
...
$I_{ii}\equiv0$ and $I_{ij}\equiv0$ and so $\det(\bm{I}(\theta))\equiv0$, and the matrix is singular. In the limit $\Lambda_i\rightarrow0$ the
variance \autoref{eq:app-variance alternative} $\var(d)\rightarrow\infty$. This is again in contrast to the original FREM computed from \autoref{eq:app-Fisher Info Pixelised - Ram}, which gives finite FREM even for one source missing (see discussion of limits in \autoref{sub:Two-sources-separated}). 
...
The expressions leads to different results for sources with unequal intensity. If $\Lambda_2=\alpha\Lambda_1$, then can be shown, that for negligible values of $b/\Lambda_2$ the ratio between the original FREM and our proposed formula for $d\rightarrow\infty$ is $2\sqrt{2}/3$.

%==========================================
%==========================================

\section{Time distribution of the intensities - averaging\label{sec:Appendix - blinking not integrated}}
We assume the likelihood dependent on parameter $\bm{\Lambda}$ (for example, $\bm{\Lambda}=(\Lambda_1,\Lambda_2)$ - intensity of two sources in the recorded frame). If we \emph{knew the configuration} of $\bm{\Lambda}$ we would write the log-likelihood
%
\begin{equation}
...
If we \emph{do not know the configuration} of $\bm{\Lambda^\alpha}$ in each frame, then we have to rely only on the distribution $p(\Lambda)$ and integrate over it within the likelihood function:
%
\begin{equation}
...
%
If $c^1=c^2$ then $\lambda^{\alpha=1}=\lambda^{\alpha=2}$ and $\frac{\partial\Po(\lambda^{\alpha=1})}{\partial c^1}=\frac{\partial\Po(\lambda^{\alpha=2})}{\partial c^2}$.
...
\end{equation}
%
where $q_k$ and $q'_k$ are pixelated PSF and derivative, respectively. 
