...
The motif of this thesis is the resolution of an optical microcope. We present two different techiques providing images with resolution higher than the diffraction limit and demonstrate them on real biological samples. In the theoretical part of the thesis we discuss the effect of noise and fluorescence intermittency on the resolution limit. 

After a general introducion in this chapter, we discuss a new approach in the localiation microcopy in Ch1\fixme{ref. to ch1}. Application of established machine learning technique of non-negative matrix factorisation (NMF) enables us to computationlly separate images of individual blinking fluorophores depsite their high mutual overlap in the recorded data. We show that with this approcach we can use quantum dots (QDs) as a fluorescent labels for super-resolution microcopy. QDs are extremely bright and stable fluorophores and the single QDs can be recorded using a standard CCD camera even withou EM enancement. Data acquisiton is therefore simple and does not require any specialsed hardware. It can be performed on any wide-field epi-fluorescence microscope equipped with camera which makes this super-resolution method very accesible to wide scientific community. 

Ch2 \fixme{ref. to ch2} discusses the resolution criterion for noisy and pixelised data and shows that intermittency of the source's brightness (blinking) can be beneficial and provide higher resolution when compared to sources with static intensity. 

The last chapter Ch3\fixme{ref. to ch3} we present structured illumination microscopy (SIM) combined with line-scanning (LS). Our goal is to introduce the SIM into more realistic and biologically relevant settings. This hig-resolution technique reduces the out-of-focus fluorescence background, which improves the modulation and the quality of the illumination pattern. The method enables resolution improvement in relatively thick and sensely labeled fluorescent samples, which opens SIM-based methodsto wider range of biological research. The reconstructed images reveal high details of the specimen’s inner structure and suffer less from the artefacts when compared to the conventional SIM methods.  
...
There are many different types of optical microcopes. The most common is observation of the specimen with transmited, reflected or fluorescent light. This thesis focusses on fluorescence microcopy, which is discussed further in \autoref{sec:Fluorescence microcopy}.
...
The emergence of new microcopy methods surpassing the classical resolution limit (super-resolution microcopy) at the end of 20\ths\ century and the beginning of 21\st\ century has given another spin to the optical microscopy research. The resolution of the super-resolution optical microcopes has reached the order of ten nanometers and some researchers propse the term ``optical nanoscopy'' to be used. However, super resolution micro/nano scopy remains a challenging task, especially when applied to living biological specimens. Two different super-resolutin techniques are discussed in this thesis. 
...
\end{equation}
%
where $Z$ is a normalizing constant, $J_1$ is the Bessel function of the first kind of order one and $\rho$ is the distance from the center of the image. $\alpha$ depends on the emission wavelength $\lambda_{em}$ and the numerical apperture of the objective $\unit{NA}$:
%
\begin{equation}
...
The resolution limit \autoref{eq:Airy} relates to the noise-free situation with continuous representation of the PSF. The pixelisation and noise associated with photon detection process can alter the ``resolution'' significantly. The influence of the noise, pixelisation or brightness intermittency to the resolution limit is discussed in \autoref{sec:Theoretical-limits-of the LM}. 
